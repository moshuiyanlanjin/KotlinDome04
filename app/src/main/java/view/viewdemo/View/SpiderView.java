...
public class SpiderView extends View {
    ...
    //连接线的点
    private float[] lines = {3.5f,5,4,6,2,1};
    //假设最大的长度值
    ...
    private void connecting_lineview(Canvas canvas) {
        ...
        for (int i = 0;i < lines.length;i++){
            float gx = (float) (x + radius * Math.sin(Math.PI*2/lines.length*i)*lines[i]/maxline);
            float gy = (float) (y + radius * Math.cos(Math.PI*2/lines.length*i)*lines[i]/maxline);
            if(i == 0){
                path.moveTo(gx,y);
            }else{
                path.lineTo(gx,gy);
    ...
    private void cut_off_ruleview(Canvas canvas) {
        ...
        for (int i = 0;i<metre;i++){
            path.reset();
            path.moveTo(x,y);
            float gx = (float) (x + radius * Math.sin(Math.PI*2/metre*i));
            float gy = (float) (y + radius * Math.cos(Math.PI*2/metre*i));
            path.lineTo(gx,gy);
        ...
        for (int i = 1;i<=tier;i++){
            ...
            for (int j = 0; j < metre;j++){
                if(j == 0){
                    ...
                    float gx = (float) (x + cur * Math.sin(Math.PI*2/metre*j));
                    float gy = (float) (y + cur * Math.cos(Math.PI*2/metre*j));
                    path.lineTo(gx,gy);
                }
            }
            path.close();
